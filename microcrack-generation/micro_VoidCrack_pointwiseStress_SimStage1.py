...
class microvoid:
    ...
    def stress_state(self, frameOref, MF_origin_x, MF_origin_y, dir_n):
        ...
        if frameOref == 'Eulerian':
            ...
            x_mc = x_mc-MF_origin_x
            y_mc = y_mc-MF_origin_y
    ...
    def next_XYpoint(self, dt, frameOref, MF_origin_x, MF_origin_y, dir_n, sField_bbox_coords):#XX,YY,sigma_1, rot_to_principal_dir, 
        ...
        # So just return the previous location the microcrack crack tip. This will be returned automatically if velocity is 0.
            ...
            # Increase the total length of the microcrack by the additional length attained in this iteration.
            # self.microCrack_length += (mc_V[0] + mc_V[1])*dt
    ...
    def mc_effectiveGeom(self):
        ...
        self.a_eff = 0.5*abs(self.x_vals[0,-1] - self.x_vals[1,-1]) # This approximation is reasonable when approach 1 is being used
        ...
        if self.x_vals[0,-1] != self.x_vals[1,-1]:
            self.inclAng = np.arctan((self.y_vals[0,-1] - self.y_vals[1,-1])/(self.x_vals[0,-1] - self.x_vals[1,-1])) #+ np.pi # DO NOT Increase inclAng angle by pi so that the MC local +ve x-axis is point back towards MF - we only do this when calculating interaction stresses
        else:
    ...
    '''METHOD FOR USE DIRECT IN MAIN SCRIPT'''
    @staticmethod
    def global_MF_to_local_stresses2(inclAng, sigma_xx, sigma_yy, sigma_xy, dir_n, frameOref):
        
        # Angle of inclination of local x-axis wrt Main Fracture X-axis. -pi/2 < theta_local <= pi/2
        
        if frameOref == 'Eulerian':
            theta_local = inclAng - dir_n  # Eulerian
        elif frameOref == 'Lagrangian':
            theta_local = inclAng          # Lagrangian
    ...
    '''METHOD FOR USE DIRECT IN MAIN SCRIPT'''
    @staticmethod
    def local_to_global_MF_stresses2(inclAng,sigma_xx, sigma_yy, sigma_xy, dir_n, frameOref):
    ...
    @staticmethod
    def interaction2(x_mc, y_mc, L_mc, mc_orientation, x_int_loc, y_int_loc, MF_hlf_Ln, sigma_a, V_MF):
        
        inclAng = mc_orientation + np.pi
        dir_n=0 # APPROACH 1
        frameOref = 'Lagrangian' # Used for Simulation Stage 1
        a_mc = L_mc/2
        ...
        # Change coordinate reference system of stress point (x,y) from global CRS to local MC CRS
        # Note: self.inclAng is the 
        x_local, y_local = microvoid.change_CRS_geom(x_int_loc, y_int_loc, x_mc, y_mc, inclAng)         # WHAT ABOUT FRAME OF REFERNECE HERE?
        
        
        # Calculate stress applied to each mC
        sigma_xx_av, sigma_yy_av, sigma_xy_av, __, __ = stresses.stress_Yoffe_MC(x_mc, y_mc, MF_hlf_Ln, sigma_a[1], V_MF, microvoid.rho_s, microvoid.G, microvoid.nu)
        
        # Convert applied stresses from Global to LOCAL COORDINATES - i.e. Convert from Instantaneous MF to local MC CRS
        sigma_xx_av_local, sigma_yy_av_local, sigma_xy_av_local = microvoid.global_MF_to_local_stresses2(inclAng, sigma_xx_av, sigma_yy_av, sigma_xy_av, dir_n, frameOref)
        ...
        if sigma_yy_av_local >= 0.: #(sigma_yy_av_local > 0.) & (sigma_xy_av_local != 0.): # Consider crack mode I and II
            ...
            # Calculate stresses resulting from Mode I loading - Griffith (Williams) Stress Soln
            sigma_xx_interaction_I, sigma_yy_interaction_I, sigma_xy_interaction_I, __, __ = stresses.stress_Griff_MC(x_local, y_local, a_mc, sigma_yy_av_local, microvoid.nu)
            sigma_xx_interaction_II, sigma_yy_interaction_II, sigma_xy_interaction_II, __, __ = stresses.stress_Griff_II_MC(x_local, y_local, a_mc, sigma_xy_av_local, microvoid.nu)
        ...
        # Convert stresses from local coordinates to GLOBAL coordinates?? DOESNT THIS TRANSFER STRESSES TO INSTANTANEOUS MF STRESSES
        sigma_xx_interaction_global, sigma_yy_interaction_global, sigma_xy_interaction_global = microvoid.local_to_global_MF_stresses2(inclAng, sigma_xx_interaction, sigma_yy_interaction, sigma_xy_interaction, dir_n, frameOref)
